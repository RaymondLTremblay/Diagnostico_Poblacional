...
Elasticidad_estadios =tribble(~Especie, ~Fecundidad, ~Stasis, ~Crecimiento, ~Plántulas, ~No_reproductivos, ~Reproductivos,
...
EE=Elasticidad_estadios %>% 
  select(Especie, Fecundidad, Stasis, Crecimiento) %>%
  pivot_longer(cols = -Especie, names_to = "Estadios", values_to = "Elasticidad")

EE$Estadios=factor(EE$Estadios, levels = c("Fecundidad", "Stasis", "Crecimiento"))
  ...
  labs(title = "Elasticidad por estadios de historia de vida",
       x = "Estadios",
       y = "Elasticidad")+
...
EE1$Estadios=factor(EE1$Estadios, levels = c("Plántulas", "No_reproductivos", "Reproductivos"))
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Elasticidad por etapas",
       x = "Etapas",
       y = "Elasticidad")+
  scale_fill_discrete(guide =
      guide_legend(label.theme = element_text(angle = 0, face = "italic")))
